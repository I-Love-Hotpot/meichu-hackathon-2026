  ...
  const screenConfig = (() => {
    switch (screen) {
      ...
      case SCREEN.EMERGENCY:
        return {
          title: t('emergency.title'), count: 3,
          left: t('common.open'), right: t('common.back'),
          emergency: true,
          onEnter: () => {
          ...
          content: <div className="dense-list">
            <ListRow label={`1  ${t('emergency.medicationCategory')}`} state="danger" selected={focus === 0} onClick={() => navigate(SCREEN.MEDICATION_EMERGENCY, 0)} />
            <ListRow label={`2  ${t('emergency.firstAidCategory')}`} selected={focus === 1} onClick={() => navigate(SCREEN.FIRST_AID, 1)} />
            <ListRow label={`3  ${t('emergency.cprCategory')}`} state="danger" selected={focus === 2} onClick={() => {
              setCprStep(0)
              navigate(SCREEN.CPR_GUIDE, 2)
            }} />
          </div>,
      ...
      case SCREEN.MEDICATION_EMERGENCY:
        return {
          title: t('emergency.medicationTitle'), count: 1,
          left: '', right: t('common.back'), emergency: true,
          content: <div className="emergency-detail">
            <p className="emergency-detail__label">{t('emergency.allergyLabel')}</p>
            <p className="emergency-result">{t('emergency.allergyValue')}</p>
            <p className="emergency-detail__label">{t('emergency.currentMedicationLabel')}</p>
            <p>{t('emergency.currentMedicine')}</p>
            <p className="emergency-warning">{t('emergency.medicationWarning')}</p>
          </div>,
        }

      case SCREEN.FIRST_AID:
        return {
          title: t('emergency.firstAidTitle'), count: 2,
          left: t('common.open'), right: t('common.back'), emergency: true,
          onEnter: () => navigate(focus === 0 ? SCREEN.FIRST_AID_WOUND : SCREEN.FIRST_AID_PREGNANCY),
          onNumber: (number) => {
          ...
          content: <>
            ...
            <ListRow label={`1  ${t('emergency.woundTitle')}`} selected={focus === 0} onClick={() => navigate(SCREEN.FIRST_AID_WOUND, 0)} />
            <ListRow label={`2  ${t('emergency.pregnancyTitle')}`} selected={focus === 1} onClick={() => navigate(SCREEN.FIRST_AID_PREGNANCY, 1)} />
          </>,
        }

      case SCREEN.FIRST_AID_WOUND:
        return {
          title: t('emergency.woundTitle'), count: 1,
          left: '', right: t('common.back'), emergency: true,
          content: <div className="emergency-detail">
            <ol>
              {[1, 2, 3, 4].map((step) => <li key={step}>{t(`emergency.woundSteps.step${step}`)}</li>)}
            </ol>
      ...
      case SCREEN.FIRST_AID_PREGNANCY:
        return {
          title: t('emergency.pregnancyTitle'), count: 1,
          left: '', right: t('common.back'), emergency: true,
          content: <div className="emergency-detail">
            <ol>
              {[1, 2, 3].map((step) => <li key={step}>{t(`emergency.pregnancySteps.step${step}`)}</li>)}
            </ol>
            <p className="emergency-warning">{t('emergency.pregnancyWarning')}</p>
          </div>,
        }
      ...
      case SCREEN.CPR_GUIDE: {
        ...
        const advanceCpr = () => {
          ...
          else setCprStep((current) => Math.min(current + 1, CPR_STEP_COUNT - 1))
        }
        return {
          title: t('emergency.cprTitle'), count: 1,
          left: isLastCprStep ? t('common.mainMenu') : t('emergency.nextStep'),
          right: t('common.back'), emergency: true,
          onEnter: advanceCpr,
          onLeft: advanceCpr,
          content: <div className="cpr-step" aria-live="polite">
            <p className="cpr-step__progress">{t('emergency.stepProgress', { current: cprStep + 1, total: CPR_STEP_COUNT })}</p>
            <strong>{t(`emergency.cprSteps.step${cprStep + 1}Title`)}</strong>
            <p>{t(`emergency.cprSteps.step${cprStep + 1}Body`)}</p>
            {cprStep === 3 && <p className="cpr-step__tempo">100–120 / min</p>}
