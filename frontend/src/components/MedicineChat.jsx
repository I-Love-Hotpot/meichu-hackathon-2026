  ...
  async function search(event) {
    ...
    try {
      const response = await fetch(
        `${apiBase}/api/medicine/search?q=${encodeURIComponent(query)}`,
        { signal: controller.signal },
      );
      const data = await response.json();
      if (!response.ok || !data.ok)
        throw new Error(
          data.error || "Medicine search is temporarily unavailable.",
        );
      ...
      if (searchRef.current === controller)
        setSearchError(
          failure.name === "AbortError"
            ...
            : "Cannot search medicines right now. Please try again.",
        );
    } finally {
  ...
  async function send(event) {
    ...
    try {
      ...
      if (data.catalog?.status === "matched" && data.sources?.length === 1)
        setSelectedMedicine(data.sources[0]);
      setQuestion("");
    } catch (failure) {
      setError(
        failure.name === "AbortError"
  ...
  return (
    <section className="medicine-chat" aria-label="Medicine question chat">
      <div className="chat-heading">
        <div>
          <h2>Medicine Questions</h2>
          <p>Understand medicine labels and appearance data</p>
        </div>
        <button type="button" onClick={clearChat} disabled={sending}>
          Clear chat
        </button>
      </div>
      <details className="recognition-input catalog-search" open>
        ...
        {selectedMedicine ? (
          <div className="selected-medicine">
            ...
            <button type="button" onClick={clearChat} disabled={sending}>
              Change medicine and clear chat
            </button>
          </div>
        ) : (
          <>
            <form onSubmit={search}>
              <label htmlFor="medicine-search">
                Medicine name or license identifier
              </label>
              <div className="chat-compose">
                <input
                  ...
                  placeholder="Example: SODIUM BICARBONATE"
                />
                <button
                  type="submit"
                  disabled={
                    sending || searching || searchQuery.trim().length < 2
                  }
                >
              ...
              <div aria-live="polite">
                <p>
                  ...
                  {searchResult.total > searchResult.records.length
                    ? `. Showing the first ${searchResult.records.length}; add the strength or formulation to narrow the search.`
                    : "."}
                </p>
        ...
        <p>
          Source: 42_2.csv. It contains names and appearance data, but no
          indications, adverse effects, or dosage.
        </p>
      </details>
      <details className="recognition-input">
        <summary>
          Add recognition data{" "}
          {recognitionText.trim() ? "(added)" : "(optional)"}
        </summary>
        <label htmlFor="recognition-text">
          Paste recognized text or package-label content
        </label>
        <textarea
          id="recognition-text"
          rows={3}
        ...
        />
        <p>
          Recognition results must still be checked against the package or by a
          pharmacist.{" "}
          {messages.length > 0 &&
            "Clear the chat before replacing this content."}
        </p>
      </details>
      <div
      ...
      >
        <article className="chat-bubble assistant">
          ...
          <p>
            Welcome. You can ask in any language; every response will use
            American English. Search and select a medicine, or include its name
            in your question.
          </p>
        </article>
        {messages.map((item, index) => (
          <article key={index} className={`chat-bubble ${item.role}`}>
            <strong>
              {item.role === "user" ? "You" : "Medicine Data Assistant"}
            </strong>
            ...
            {item.reply?.warnings.length > 0 && (
              <div className="chat-warnings">
                ...
                <ul>
                  {item.reply.warnings.map((warning, i) => (
                    <li key={i}>{warning}</li>
                  ))}
                </ul>
              </div>
            ...
            {item.reply?.followUpQuestions.length > 0 && (
              <div className="chat-followups">
                ...
                <ul>
                  {item.reply.followUpQuestions.map((followup, i) => (
            ...
            {item.sources?.length > 0 && (
              <details
                ...
                open={item.catalog?.status === "ambiguous"}
              >
                <summary>
      ...
      <p className="chat-privacy">
        Messages and recognition data are sent to Gemini. Chinese CSV fields are
